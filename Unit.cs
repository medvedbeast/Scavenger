    ...
    public class Unit : MonoBehaviour
    {
        ...
        public float turnSpeed
        {
            get
            {
                ...
                for (int i = 0; i < modules.Count; i++)
                {
                    if (modules[i].referencedObject.type == Modules.MOVEMENT_CONTROL)
                    {
                        if (modules[i].referencedObject.hp > 0)
                            tmp += (modules[i].referencedObject as MovementControl).turnSpeed;
                    }
                }
                return tmp;
            }
        ...
        void Start()
        {
            ...
            if (this.type != Units.LOOTABLE)
            {
                for (int i = 0; i < modules.Count; i++)
                {
                    modules[i].referencedObject = Functions.Deserialize<Module>(string.Format("{0}/{1}.module", gameObject.name, modules[i].name));
            ...
            else
            {
                ...
                string[] files = System.IO.Directory.GetFiles(System.Environment.CurrentDirectory + Functions.pathToItems + "\\Items\\Modules\\SCA-01", "*.module");
                ...
                {
                    inventory.Add(Functions.Deserialize<Module>("SCA-01/" + f[r.Next(0, f.Count - 1)]));
                }
            }
        }
